...
public class T02_ListSortTest {
	 ...
	 *    정렬기준을 별도로 구현하고 싶을 때는 Comparator를 구한하여 사용
	...
	public static void main(String[] args) {
		...
		// 정렬방식을 변경하려면 정렬방식을 결정하는 객체를 만들어서 Collection.sort()메서드에 변수로 넘겨주면 된다.
 ...
 * 이 Comparator 인터페이스는 compare()라는 메서드를 재정의 하여 구현하면 된다.
 */
class Desc implements Comparator<String> {
	/**
	 * compare()메서드의 변환값을 결정하는 방법
	 * => 이 메서드가 양수를 반환하며 두 값의 순서가 바뀐다.(오름차순이 기본임)
	 * 
	 * 오름차순 정렬일 경우
	 *  => 앞의 값이 크면 양수, 같으면 0, 앞의 값이 작으면 음수를 반환하도록 한다.
	 * 
	 * String 객체에는 정렬을 위해서 compareTo()메서드가 구현되어 있는데 이 메서드의 반환값은 오름차순에 맞게
	 * 반환되도록 구현되어 있다.
	 * (Wrapper클래스의 Date, File클래스에도 구현되어 있다)
	 */
	
	// 선언만 되어있는 부분을 오버라이드 하여 구현부를 만들어 준다.
	@Override
